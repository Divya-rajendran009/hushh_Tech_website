...
test.describe("/metrics metric card padding proof", () => {
  for (const viewport of [
    { name: "mobile", width: 390, height: 900, expectedPadding: "16px" },
    { name: "desktop", width: 1440, height: 1100, expectedPadding: "20px" },
  ]) {
    test(`uses resolved metric card padding on ${viewport.name}`, async ({
      page,
    }, testInfo) => {
      ...
      await expect(firstMetricCard).toHaveCSS("padding-left", viewport.expectedPadding);
      await expect(firstMetricCard).toHaveCSS("padding-right", viewport.expectedPadding);
      await expect(firstMetricCard).toHaveCSS("padding-top", viewport.expectedPadding);
      await expect(firstMetricCard).toHaveCSS("padding-bottom", viewport.expectedPadding);

      await page.screenshot({
        path: testInfo.outputPath(`metrics-card-padding-${viewport.name}.png`),
        fullPage: true,
      });
    });
  }
