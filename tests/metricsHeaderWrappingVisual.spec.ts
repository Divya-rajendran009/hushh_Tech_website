...
async function expectHeaderNotClipped(page: Page, text: string) {
  const header = page.getByText(text, { exact: true }).first();
  ...
  expect(geometry.headerWidth).toBeLessThanOrEqual(geometry.parentWidth + 1);
  expect(geometry.scrollWidth).toBeLessThanOrEqual(geometry.clientWidth + 1);
}

test.describe("/metrics analytics header wrapping proof", () => {
  for (const viewport of [
    { name: "mobile", width: 390, height: 900 },
    { name: "desktop", width: 1440, height: 1100 },
  ]) {
    test(`keeps analytics headers readable on ${viewport.name}`, async ({
      page,
    }, testInfo) => {
      ...
      await expectHeaderNotClipped(page, "Onboarding step distribution");
      await expectHeaderNotClipped(page, "Search appearance");
      await expectHeaderNotClipped(page, "Traffic context");

      await page.screenshot({
        path: testInfo.outputPath(`metrics-header-wrapping-${viewport.name}.png`),
        fullPage: true,
      });
    });
  }
